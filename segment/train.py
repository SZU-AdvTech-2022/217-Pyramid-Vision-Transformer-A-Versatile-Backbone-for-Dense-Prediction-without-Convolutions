...
from utils.dataset_ import VOCSegDataset as Dataset
from utils.evaluator import Evaluator
# from segment.model.my_PVT_seg import segmentation as Model
# config = CONFIGS['PVT_seg_small']
...
def main(args):
    ...
    train_data_loader = Dataset(is_train=True, crop_size=(224, 224), voc_root=data_dir).set_attrs(batch_size=batch_size,shuffle=True)
    val_data_loader = Dataset(is_train=False, crop_size=(224, 224), voc_root=data_dir).set_attrs(batch_size=batch_size,shuffle=True)
    # train_data_loader = CIFAR10(train=True, transform=None)
    # val_data_loader = CIFAR10(train=False, transform=None)
